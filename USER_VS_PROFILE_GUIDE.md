...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_BASE}/profiles/${PORTFOLIO_OWNER_PROFILE_ID}/`).then(r => r.json()),
      fetch(`${API_BASE}/projects/?profile=${PORTFOLIO_OWNER_PROFILE_ID}`).then(r => r.json()),
      fetch(`${API_BASE}/experiences/by_profile/${PORTFOLIO_OWNER_PROFILE_ID}/`).then(r => r.json()),
      fetch(`${API_BASE}/education/by_profile/${PORTFOLIO_OWNER_PROFILE_ID}/`).then(r => r.json()),
      fetch(`${API_BASE}/skills/by_profile/${PORTFOLIO_OWNER_PROFILE_ID}/`).then(r => r.json()),
      fetch(`${API_BASE}/certifications/active/?profile=${PORTFOLIO_OWNER_PROFILE_ID}`).then(r => r.json()),
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    // Fetch your profile (content to edit)
    fetch(`http://localhost:8000/api/profiles/${PORTFOLIO_OWNER_PROFILE_ID}/`, {
...
### Mistake 5: Adding auth headers to public endpoints
```typescript
// ❌ WRONG - Public endpoints don't need auth
fetch('/api/projects/', {
  headers: { 'Authorization': `Bearer ${token}` }
});

// ✅ CORRECT - No auth needed for viewing
fetch('/api/projects/'); // Just fetch!
```
...
- [x] Create homepage that fetches profile by UUID
- [x] Display profile information (name, bio, image)
- [x] Show social links from profile
- [x] List all projects (`GET /api/projects/?profile={uuid}`)
- [x] Display work experiences (`GET /api/experiences/by_profile/{uuid}/`)
- [x] Show education history (`GET /api/education/by_profile/{uuid}/`)
- [x] Display skills (`GET /api/skills/by_profile/{uuid}/`)
- [x] Show certifications (`GET /api/certifications/active/?profile={uuid}`)
- [ ] Add anonymous contact form (`POST /api/contacts/submit/` - NO auth required!)
- [x] All endpoints are PUBLIC - no authentication needed!

### For Admin Pages (Login Required)
- [x] Create login page using `/api/auth/login/`
- [x] Store JWT tokens in localStorage
- [x] Create protected route component
- [x] Build profile edit form (PATCH `/api/profiles/{uuid}/`)
- [x] Build project creation form (include profile UUID!)
- [x] Add image upload functionality
- [x] Implement logout (clear tokens)

### Configuration
- [x] Add profile UUID constant to your codebase
- [x] Set up API base URLs (localhost:8000)
- [ ] Configure CORS if needed
- [x] Add error handling for 401/403/404
- [x] Replace system alerts with toast notifications
- [x] Add routing for public pages
...
GET    /api/profiles/{uuid}/         - Get specific profile (PUBLIC)
PATCH  /api/profiles/{uuid}/         - Update profile (admin only)
GET    /api/profiles/{uuid}/social-links/  - Get social links (PUBLIC)
```
...
GET    /api/projects/{uuid}/         - Get project details (PUBLIC)
...
GET    /api/experiences/by_profile/{profile_uuid}/  - Get by profile (PUBLIC)
```
...
GET    /api/education/by_profile/{profile_uuid}/    - Get by profile (PUBLIC)
```
...
GET    /api/skills/by_profile/{profile_uuid}/       - Get by profile (PUBLIC)
GET    /api/skills/by_category/      - Get grouped by category (PUBLIC)
```
...
GET    /api/certifications/by_profile/{profile_uuid}/  - Get by profile (PUBLIC)
GET    /api/certifications/active/   - Get non-expired certs (PUBLIC)
```

### Contact Endpoints (Anonymous Contact Forms)
```
POST   /api/contacts/submit/         - Submit contact form (PUBLIC - NO AUTH!)
GET    /api/contacts/messages/       - List contact messages (admin only)
GET    /api/contacts/messages/{uuid}/ - Get message details (admin only)
PATCH  /api/contacts/messages/{uuid}/ - Update message status (admin only)
DELETE /api/contacts/messages/{uuid}/ - Delete message (admin only)
GET    /api/contacts/messages/statistics/ - Get stats (admin only)
```
...
5. **Test public access** - Verify all portfolio content is viewable without login
...
- **"Do visitors need to login?"** → NO! All portfolio content is public
...
2. Verify you're using the correct profile UUID
3. Make sure authentication token is included ONLY for admin operations
4. Confirm public endpoints don't have Authorization headers
